...
- 用户**本次会话明确要求**跳过三方向（「不用出三版」「只做一版」），或明确沿用已批准方向（「就按上次那个方向」）。「直接做」「快点」及时间限制本身不构成豁免；明确只做一版时按一版执行。
...
**Phase 1 · 核对需求与参考，澄清必要缺口**
先核对现有材料中的目标受众 / 核心信息 / 情感基调 / 输出格式，仅对影响结果且无法确定的缺口提问（一次最多 3 个问题）；信息已齐全则继续。
**同时核对参考材料**：先检查已有项目名称、品牌资产与参考；仅在仍有必要缺口时从以下问题中选取，每轮最多3个，已提供的信息不重复索要：
- 这个项目/产品**叫什么名字**？
...
1. **理解需求**：
   ...
   - 仅在现有上下文与已授权检查无法解决必要信息缺口时问 clarifying questions，详见 `references/workflow.md`；不因是新任务而重复询问已知信息。三方向选择门仍按下条执行。
   - **澄清检查点1**：先核对已有上下文，仅询问仍缺失且影响结果的信息；每轮最多3个独立问题。依赖答案的工作等待回答，不依赖答案的已授权准备可继续。
...
2. **探索资源 + 抽核心资产**（不只是抽色值）：读 design system、linked files、上传的截图/代码。**涉及具体品牌时必走 §1.a「核心资产协议」五步**，产出 `brand-spec.md`。
   - **Agent 自检2·资产检查**：开工前确认核心资产到位——实体产品要有产品图（不是 CSS 剪影）、数字产品要有 logo+UI 截图、色值从真实 HTML/SVG 抽取。缺了就停下补，不硬做。
...
7. **验证**：用Playwright截图（见 `references/verification.md`），检查控制台错误，发给用户。
   **Agent 自检5：交付前自己肉眼过一遍浏览器**。AI写的代码经常有interaction bug。
...
**检查点原则**：区分「用户批准」与「Agent 自检」。三方向选择、检查点3（五问与系统）、检查点4（对齐稿反馈）以及其他明确要求用户批准的步骤，展示具体材料后等待批准。Agent 自检由 Agent 检查、修复，通过后继续；只有发现需要用户决定的问题才询问。🛑 标记本身不新增批准要求。检查点1仅询问仍缺失且影响结果的信息，已有答案不重复问；等待时可继续不依赖答案的已授权准备。
...
信息检查清单（先查现有上下文，只问影响结果的缺口；参见 `references/workflow.md`）：
...
| 用户拒绝回答问题清单 | 用户说"不要问了，直接做" | **拒答问题≠跳过三方向**：可自行确定的细节记录为 assumption，影响正确性或范围且不能推断的缺口仍需明确。方向门及豁免统一按 Fallback「唯一豁免」执行；明确只要一版就做一版，并在 `direction-approved.md` 记录原话 |
...
| 时间紧迫要快交付 | 用户说"30分钟内要" | 缩小初稿与成品范围以适配时限；时间限制本身不豁免三方向或其他明确批准门。是否适用三方向豁免统一查 Fallback「唯一豁免」；需改变其他批准门时取得明确授权并记录 |
