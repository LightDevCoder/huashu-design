...
先检查用户已提供的要求、项目文件和参考材料。只问影响结果、且无法从上下文或已授权检查中确定的信息；没有固定问题数量。

**什么时候必须问**：仍有影响正确性、范围或验收的用户决策，且现有材料无法确定。新任务本身不构成提问理由。三方向选择与其他明确批准门继续按 `../SKILL.md` 执行。

**什么时候可以不问**：小修小补、follow-up任务、用户已经给了明确PRD+截图+上下文。

**怎么问**：大部分 agent 环境没有结构化问题 UI，在对话里用 markdown 清单问即可。**每轮最多3个独立问题，让用户批量答**，不要一来一回一个个问——那会浪费用户时间、打断用户思路。

## 信息检查清单

检查这5类信息是否已明确，只询问必要缺口：
...
下面是问题库，不是需要完整发给用户的问卷。按上下文筛选，每轮最多3个必要问题：

```markdown
请从以下问题库中选取本轮仍未明确的必要问题：
